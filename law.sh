...
arch-chroot "echo $hostname > /mnt/etc/hostname"
clear
...
arch-chroot "sed -i  '1i\en_US.UTF-8 UTF-8' /mnt/etc/locale.ge"
arch-chroot "locale-gen"
arch-chroot "echo LANG=en_US.UTF-8 > /mnt/etc/locale.conf"
arch-chroot "export LANG=en_US.UTF-8"
arch-chroot "ln -s /usr/share/zoneinfo/Singapore /mnt/etc/localtime"
...
arch-chroot "sed -i  's@autodetect modconf block@autodetect modconf block encrypt lvm2 @g' /mnt/etc/mkinitcpio.conf"
...
arch-chroot "mkdir -p /mnt/boot/grub/locale"
arch-chroot "grub-mkconfig -o /mnt/boot/grub/grub.cfg"
arch-chroot "sed -i 's@/vmlinuz-linux@/vmlinuz-linux cryptdevice=/dev/sda3:ArchSysLuks@g' /mnt/boot/grub/grub.cfg"
clear
